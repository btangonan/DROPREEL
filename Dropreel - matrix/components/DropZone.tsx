            ...
            <div className="grid grid-cols-3 gap-3 flex-1 overflow-y-auto">
              {selectedVideos.map((video) => (
                <div key={video.id} className="relative group">
                  ...
                  />
                  {/* Remove button overlay - positioned inside video box */}
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      onRemoveVideo(video.id);
                    }}
                    className="absolute top-2 right-2 w-6 h-6 bg-archive-danger border-2 border-archive-primary text-background hover:bg-archive-primary hover:text-background transition-colors opacity-0 group-hover:opacity-100 flex items-center justify-center z-10 p-0"
                  >
                    <Trash2 className="w-3 h-3 shrink-0 transform translate-x-0.5 -translate-y-0.5" />
                  </button>
                </div>
              ))}
            </div>
